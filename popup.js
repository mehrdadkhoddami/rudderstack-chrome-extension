...
// ── State ─────────────────────────────────────────────────────────────────────
// allItems: Map<messageId, itemData>  — every event ever seen this session
// sentMessageIds: Set<messageId>      — events confirmed sent via network batch
...
function saveState(tabId) {
    chrome.storage.local.set({
        [`allItems_${tabId}`]: JSON.stringify([...allItems]),
        [`sentIds_${tabId}`]: JSON.stringify([...sentMessageIds]),
    });
}
...
function loadState(tabId, callback) {
    chrome.storage.local.get([`allItems_${tabId}`, `sentIds_${tabId}`], (result) => {
        if (result[`allItems_${tabId}`]) {
            try { allItems = new Map(JSON.parse(result[`allItems_${tabId}`])); } catch(e) {}
        }
        if (result[`sentIds_${tabId}`]) {
            try { sentMessageIds = new Set(JSON.parse(result[`sentIds_${tabId}`])); } catch(e) {}
        }
        callback();
    });
}
...
// ── Ingest batch events from network (background.js intercept) ────────────────
// These are confirmed-sent events. We:
//   1. Add them to allItems if not seen before (new event → show in panel)
//   2. Mark them as sent in sentMessageIds
//   3. Update DOM for any matching existing items
function ingestBatchEvents(batchArray, timestamp) {
    if (!Array.isArray(batchArray)) {
        console.warn('[RS Panel] ingestBatchEvents called with non-array:', batchArray);
        return;
    }

    console.log(`[RS Panel] ingestBatchEvents: processing ${batchArray.length} events`);
    let newCount = 0;
    ...
    batchArray.forEach((event, i) => {
        if (!event || !event.messageId) {
            console.warn(`[RS Panel]   [${i}] Skipped — no messageId`, event);
            skippedCount++;
            return;
        }
        if (event.type !== 'track' && event.type !== 'page') {
            console.log(`[RS Panel]   [${i}] Skipped — type="${event.type}" (not track/page)`);
            skippedCount++;
            return;
        }
        ...
        if (!allItems.has(msgId)) {
            ...
            console.log(`[RS Panel]   [${i}] NEW event: "${event.event || event.name || event.type}" | msgId: ${msgId}`);
            newCount++;
        } else {
            console.log(`[RS Panel]   [${i}] Already seen — marked SENT: "${event.event || event.name || event.type}" | msgId: ${msgId}`);
            alreadySeenCount++;
        }
    });

    console.log(`[RS Panel] Done: new=${newCount}, markedSent=${alreadySeenCount}, skipped=${skippedCount} | total allItems=${allItems.size}`);

    renderAll();
    saveState(currentTabId);
}

// ── Ingest localStorage events (content script) ───────────────────────────────
// These are queued (not yet sent) events.
...
function renderAll() {
    ...
    sorted.forEach(([msgId, data], index) => {
        ...
        if (!el) {
            ...
            existingEls.delete(msgId); // still valid, keep it
            // Update sent state if changed
            if (isSent && !el.classList.contains('sent-item')) {
                el.classList.add('sent-item');
                const keyContainer = el.querySelector('.key-container');
                if (keyContainer && !keyContainer.querySelector('.sent-badge')) {
    ...
    // Remove stale elements (shouldn't normally happen)
...
function createCopyButton(value) {
    ...
    btn.addEventListener('click', () => {
        ...
        }).catch(err => { console.error('Failed to copy:', err); showToast('Failed to copy', 'error'); });
    });
    return btn;
}
...
function createItemElement(key, data, isSent) {
    ...
    const originalKey = data.originalKey || key || 'Unknown';
...
document.addEventListener('DOMContentLoaded', function() {
    getCurrentTabId((tabId) => {
        if (!tabId) { console.error('No valid tab ID found'); return; }
        currentTabId = tabId;

        document.getElementById('clearButton').addEventListener('click', () => clearAllItems(tabId));

        // Load persisted state, then do initial localStorage scan
        loadState(tabId, () => {
            // Render whatever we have from previous session
        ...
        chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
            ...
            // Accept if:
            //   • no tabId specified (broadcast)
            //   • tabId === -1  (sendBeacon — browser doesn't know which tab)
            //   • tabId matches our tab
            const msgTabId = message.tabId;
            const tabMatches = msgTabId === undefined || msgTabId === null ||
                               msgTabId === -1 || msgTabId === tabId;

            console.log('[RS Panel] updatePopup received | source:', message.source,
                        '| msgTabId:', msgTabId, '| ourTabId:', tabId,
                        '| tabMatches:', tabMatches,
                        '| items:', Array.isArray(message.data) ? message.data.length : typeof message.data);

            if (tabMatches) {
                if (message.source === 'network' && Array.isArray(message.data)) {
                    console.log('[RS Panel] ✅ Ingesting', message.data.length, 'batch events from network');
                    ingestBatchEvents(message.data, message.timestamp);
                } else if (message.source === 'localStorage' && message.data) {
                    console.log('[RS Panel] ✅ Ingesting localStorage items, keys:', Object.keys(message.data).length);
                    ingestLocalStorageItems(message.data);
                }
                if (sendResponse) sendResponse({ received: true });
            } else {
                console.log('[RS Panel] ⏭ Skipped — tabId mismatch');
            }
            return true;
        });
    });
